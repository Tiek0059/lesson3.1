...
public class Main {
    ...
    public static String[] namesOfHeroes = {"Coll", "Mag", "Zmei", "Dr.Doc", "Golem", "Kai", "Nya", "Loid"};
    ...
    public static void madicalTreatment() {
        for (int i = 0; i < heroesHp.length; i++) {
            if (heroesHp[i] <= 100 && (heroesHp[i] > 0 && heroesHp[i] != heroesHp[3] && heroesHp[3] > 0)) {
                ...
                System.out.println("Dr.Doc cured " + namesOfHeroes[i] + " and give him " + medicsHelp + ". " + namesOfHeroes[i] + " thanks for help Dr.Dog!!!");
                break;
            }
        }
    }
    ...
    public static void bossHits() {
        ...
        if (heroesHp[5] > 0 && lucky == 1 && stun != 2 ) {
            if (heroesHp[4] > 0) {
                ...
                System.out.println("Kai dodged!!!");
            } else heroesHp[5] += bossDamage;
        }
        Random rand = new Random();
        int focBattle = rand.nextInt(10) + 5;
        if (heroesHp[6] > 0 && stun != 2) {
            heroesHp[6] = heroesHp[6] + focBattle;
            bossHp -= focBattle;
            System.out.println("Nya blocked and returned: " + focBattle);
        }
        for (int i = 0; i < heroesHp.length; i++) {
            if (heroesHp[7] > 0 && stun == 2 ){
                System.out.println("Loid stunned!!!");
                break;}
            if (heroesHp[i] > 0) {
